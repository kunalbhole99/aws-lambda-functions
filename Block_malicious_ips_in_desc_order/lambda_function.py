...
def get_next_available_rule_number(existing_entries):
    ...
    available_numbers = {num for num in range(99, 0, -1)} - rule_numbers
    return max(available_numbers) if available_numbers else None
...
def lambda_handler(event, context):
    try:
        ...
        for ip in new_ips_to_add:
            ...
            if next_rule_number is not None and next_rule_number >= 1:
                ...
                try:
                    ...
                    next_rule_number -= 1  # Decrement rule number for descending order
